...
export function canPublishAccount(account, now = Date.now()) {
  return authorizationStatus(account, now) !== 'expired' && account.permissions.includes('视频/图文发布')
}
...
export function createMockAccounts(now = Date.now()) {
  const rows = [
    ...
    ['wx-main', '视频号', '企业号', '星枢生活精选', '77409126', '4.2万', 5, '枢', 'bg-emerald-50 text-emerald-700', ''],
