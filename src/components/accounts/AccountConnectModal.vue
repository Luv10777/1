...
let scanTimer
let qrGeneration = 0
const control = 'rounded-lg border border-solid border-slate-200 bg-white px-3 py-2.5 text-sm text-slate-700! transition hover:bg-slate-50 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-blue-600'
const primary = 'inline-flex items-center justify-center gap-2 rounded-lg border-0 bg-blue-600 px-4 py-2.5 text-sm font-semibold text-white! shadow-md shadow-blue-500/20 transition hover:bg-blue-500 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-blue-600 disabled:cursor-not-allowed disabled:opacity-40'
...
<template>
  <TransitionRoot appear :show="open" as="template">
    <Dialog as="div" class="relative z-[80] font-sans text-slate-800 [font-family:Inter,-apple-system,BlinkMacSystemFont,'PingFang_SC','Segoe_UI',sans-serif] [color-scheme:light]" @close="emit('close')">
      ...
      <div class="fixed inset-0 overflow-y-auto p-4">
        <div class="flex min-h-full items-center justify-center">
          <TransitionChild as="template" enter="duration-200 ease-out motion-reduce:duration-0" enter-from="scale-95 opacity-0" enter-to="scale-100 opacity-100" leave="duration-150 ease-in motion-reduce:duration-0" leave-from="scale-100 opacity-100" leave-to="scale-95 opacity-0">
            <DialogPanel class="w-full max-w-xl overflow-hidden rounded-2xl border border-solid border-slate-200 bg-white shadow-2xl">
              <header class="flex items-center justify-between border-0 border-b border-solid border-slate-100 px-6 py-5">
                <div><div class="mb-1 text-xs font-medium text-blue-600">账号授权中心 <span class="ml-2 rounded bg-slate-100 px-1.5 py-0.5 text-[10px] text-slate-500">本地演示</span></div><DialogTitle class="m-0 text-xl font-semibold text-slate-900">{{ account ? '续期账号授权' : '绑定新账号' }}</DialogTitle></div>
              ...
              <ol class="m-0 flex list-none items-center gap-2 px-6 pt-6">
                <li v-for="(label, index) in ['选择平台', '扫码授权', '确认权限']" :key="label" class="flex flex-1 items-center gap-2 text-xs" :aria-current="step === index + 1 ? 'step' : undefined">
                  <span class="grid size-7 shrink-0 place-items-center rounded-full font-semibold" :class="step >= index + 1 ? 'bg-blue-600 text-white' : 'bg-slate-100 text-slate-400'"><Check v-if="step > index + 1" :size="14" /><template v-else>{{ index + 1 }}</template></span><span class="whitespace-nowrap" :class="step >= index + 1 ? 'text-slate-800' : 'text-slate-400'">{{ label }}</span><span v-if="index < 2" class="hidden h-px flex-1 bg-slate-200 sm:block" />
                </li>
              </ol>
              <div class="min-h-80 px-6 py-6">
                <div v-if="step === 1" class="space-y-5">
                  <fieldset class="m-0 border-0 p-0">
                    ...
                    </div><p class="mb-0 mt-2 text-xs text-slate-500">{{ selectedMeta.provider }}</p>
                  </fieldset>
                  <label class="block text-sm font-medium">授权身份<select v-model="identity" :disabled="!!account" class="mt-2 block w-full rounded-lg border border-solid border-slate-200 bg-white px-3 py-2.5 text-sm text-slate-700 focus:outline-blue-600 disabled:bg-slate-50"><option v-for="item in selectedMeta.identity" :key="item">{{ item }}</option></select></label>
                  <label class="block text-sm font-medium">{{ account ? '续期账号' : '演示账号昵称' }}<input v-model="name" maxlength="30" :readonly="!!account" placeholder="输入昵称，体验绑定后的账号管理" class="mt-2 block w-full rounded-lg border border-solid border-slate-200 bg-white px-3 py-2.5 text-sm text-slate-800 outline-blue-600 read-only:bg-slate-50" /></label>
                </div>
                <div v-else-if="step === 2" class="text-center">
                  <p class="m-0 text-sm font-medium">使用{{ platform === '视频号' ? '微信' : platform }}扫码完成授权</p>
                  <p class="mb-4 mt-2 text-xs text-slate-500">演示二维码不连接官方平台，请使用下方模拟按钮。</p>
                  <div class="relative mx-auto grid size-52 place-items-center rounded-xl border border-solid border-slate-200 bg-white p-2">
                    <img v-if="qrImage" :src="qrImage" alt="本地授权演示二维码" class="size-full" />
                    <LoaderCircle v-else-if="!qrError" class="animate-spin text-blue-600 motion-reduce:animate-none" />
                    <span v-if="qrImage" class="absolute rounded-xl bg-white p-1.5"><PlatformMark :platform="platform" /></span>
                    <div v-if="!countdown || qrError" class="absolute inset-0 flex flex-col items-center justify-center gap-3 rounded-xl bg-white/95"><span class="text-sm text-slate-600">{{ qrError || '二维码已过期' }}</span><button :class="control" @click="refreshQr"><RefreshCw :size="14" class="mr-1 inline" />刷新二维码</button></div>
                  </div>
                  <p class="my-4 flex items-center justify-center gap-2 text-xs text-slate-500" role="status"><LoaderCircle class="animate-spin motion-reduce:animate-none" :size="13" />{{ scanning ? '已收到模拟扫码，正在确认身份…' : `等待扫码 · ${countdown} 秒后过期` }}</p>
                  <button :class="primary" :disabled="!qrImage || !countdown || scanning" @click="simulateScan"><Smartphone :size="16" />模拟扫码成功</button>
                </div>
                <div v-else class="space-y-3">
                  <p class="m-0 rounded-lg bg-emerald-50 px-3 py-3 text-sm text-emerald-700"><Check class="mr-1 inline" :size="16" />{{ name }} · 模拟身份确认成功</p>
                  <p class="text-xs leading-5 text-slate-500">勾选允许平台使用的权限。取消发布权限后，该账号将不出现在内容发布页。</p>
                  <label v-for="permission in options" :key="permission" class="flex cursor-pointer items-center gap-3 rounded-lg border border-solid border-slate-200 p-3 text-sm hover:border-blue-300"><input v-model="permissions" type="checkbox" :value="permission" class="size-4 accent-blue-600" />{{ permission }}<span v-if="permission.includes('发布') || permission === '门店POI关联'" class="ml-auto text-xs text-slate-400">核心权限</span></label>
                </div>
              </div>
              <footer class="space-y-4 border-0 border-t border-solid border-slate-100 bg-slate-50/70 px-6 py-4">
                ...
                <p class="m-0 flex gap-2 text-[11px] leading-5 text-slate-500"><ShieldCheck class="mt-0.5 shrink-0" :size="14" />授权过程严格遵守官方开放平台数据安全协议，平台不会获取您的账号登录密码。</p>
              </footer>
            </DialogPanel>
          </TransitionChild>
        </div>
