...
const notice = ref('')
const orgRows = [
  { level: 'HEADQUARTERS', name: '一方志茶饮集团总部', policy: '品牌语气 / 价格事实锁定', state: '已继承' },
...
const runSandbox = () => {
  const app = createOAuthApplication({ id: 'app_sandbox_preview', developerId: 'dev_preview', name: '一方志沙箱应用', environment: 'SANDBOX', requestedScopes: scopes })
...
<template>
  <div class="ecosystem-page">
    ...
    <section v-if="activeTab === 'developer'" class="ecosystem-detail developer-layout"><article class="panel"><div class="panel-heading"><div><p class="eyebrow">SANDBOX APPLICATION</p><h3>一方志沙箱应用</h3></div><span class="sandbox-state">ACTIVE</span></div><div class="app-meta"><span>环境 <b>Sandbox</b></span><span>应用 ID <b>app_sandbox_preview</b></span></div><div class="scope-list"><p class="eyebrow">APPROVED SCOPES</p><span v-for="scope in scopes" :key="scope">{{ scope }} <i>授权</i></span></div><button class="secondary-button" type="button" @click="notice = 'Token 已在 Sandbox 中撤销，生产凭证不会在页面显示。'">撤销沙箱 Token</button></article><article class="panel"><div class="panel-heading"><div><p class="eyebrow">DEVELOPER GUARDRAILS</p><h3>开放平台尚未开放</h3></div><span class="coming-badge">WAITING FOR API</span></div><div class="guard-list"><div><b>01</b><span>OAuth + PKCE</span><i>待服务端</i></div><div><b>02</b><span>Webhook HMAC</span><i>待 Worker</i></div><div><b>03</b><span>游标分页 / 幂等</span><i>契约已定义</i></div><div><b>04</b><span>生产 Scope 审核</span><i>待权限</i></div></div></article></section>
