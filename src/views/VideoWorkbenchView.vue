...
const generate = () => {
  ...
  notice.value = isGenerating.value ? '方志编撰中，请稍候。' : '已暂停本次生成，可随时继续。'
}
    ...
    <div class="video-workbench-sidebar w-[420px] h-full flex flex-col bg-white border-r border-gray-100 shadow-[4px_0_24px_rgba(0,0,0,0.02)] z-20">
      <div class="video-workbench-scroll flex-1 overflow-y-auto p-8 space-y-8">
        <div class="video-form-section">
          ...
          <button class="video-upload-zone w-full h-32 bg-gray-50 rounded-2xl border border-gray-200 hover:border-cinnabar-300 hover:bg-cinnabar-50/30 transition-all flex flex-col items-center justify-center gap-2 group" type="button" @click="fileInput?.click()" @dragover.prevent @drop="handleDrop">
