...
function saveFaqSet() {
  ...
  const item = { id, name, type: 'faq', icon: MessageCircleQuestion, tone: 'violet', meta: `KB-${Date.now()} · 刚刚创建`, metric: '0 个问答对', description: description || '自定义 FAQ 知识集', apps: ['客服 Agent'], status: 'draft', statusLabel: '草稿', sync: '待添加问答' }
...
function restoreArchived(item) {
  ...
  customKnowledge.value.unshift({ id: item.id, name: item.name, type: item.type === '常见问答' ? 'faq' : 'rag', icon: item.type === '常见问答' ? MessageCircleQuestion : FileText, tone: item.type === '常见问答' ? 'violet' : 'blue', meta: `KB-${item.id} · 已恢复`, metric: item.metric, description: '从归档中恢复的知识集', apps: ['客服 Agent'], status: 'draft', statusLabel: '草稿', sync: '已恢复，待发布' })
